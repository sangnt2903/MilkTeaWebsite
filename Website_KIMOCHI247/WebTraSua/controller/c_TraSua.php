<?php
 ...
 class C_TraSua{
 	...
 	function themSanPham($tenhh,$hinh,$tomtat,$giatien,$soluong){
 		$m_TraSua = new M_TraSua();
 		$themSP = $m_TraSua->themTraSua($tenhh,$hinh,$tomtat,$giatien,$soluong);
 		$_SESSION['them_sp'] = "Đã thêm sản phẩm thành công !";
 		header('location:'.$_SERVER['HTTP_REFERER']);
 	}

 	function suaSanPham($mahh,$tenhh,$hinh,$tomtat,$giatien,$soluong){
 		$m_TraSua = new M_TraSua();
 		$themSP = $m_TraSua->suaTraSua($mahh ,$tenhh,$hinh,$tomtat,$giatien,$soluong);
