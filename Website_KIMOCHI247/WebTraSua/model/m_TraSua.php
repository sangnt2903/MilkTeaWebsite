<?php
	...
	class M_TraSua extends database {
		...
		function themTraSua($tenhh,$hinh,$tomtat,$giatien,$soluong){
			$sql = "INSERT INTO tb_products(tenhh,hinh,tomtat,giatien,soluong) VALUES(?,?,?,?,?)";
			$this->setQuery($sql);
			return $this->execute(array($tenhh,$hinh,$tomtat,$giatien,$soluong));
		}

		function suaTraSua($mahh,$tenhh,$hinh,$tomtat,$giatien,$soluong){
			$sql = "UPDATE tb_products SET tenhh = '$tenhh' , hinh = '$hinh', tomtat = '$tomtat', giatien = '$giatien',soluong = '$soluong' WHERE mahh = $mahh";
			$this->setQuery($sql);
			return $this->execute(array($tenhh,$hinh,$tomtat,$giatien,$soluong));
		}
